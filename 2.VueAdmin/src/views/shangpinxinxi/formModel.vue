...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.shangpinxiangqing = res.data.data.shangpinxiangqing?(res.data.data.shangpinxiangqing.replace(reg,'../../../cl2058845/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		context?.$http({
			...
			if(json.hasOwnProperty('dizhi')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.dizhi = json.dizhi
				disabledForm.value.dizhi = true;
			}
		})
		context?.$http({
